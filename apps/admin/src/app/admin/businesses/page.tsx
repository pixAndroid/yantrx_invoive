...
import { Search, Building2, Users, FileText, AlertCircle, RefreshCw, Filter } from 'lucide-react';
  ...
  const fetchBusinesses = useCallback(async () => {
    ...
    try {
      ...
      const filtered = search
        ? res.data.filter(b => b.name.toLowerCase().includes(search.toLowerCase()) || b.owner.name.toLowerCase().includes(search.toLowerCase()))
        : res.data;
        ...
        <table className="w-full">
          <thead className="bg-gray-800/50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Invoices</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Customers</th>
          ...
          <tbody className="divide-y divide-gray-800">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-12 text-center"><Building2 className="h-12 w-12 text-gray-700 mx-auto mb-3" /><p className="text-gray-500">No businesses found</p></td></tr>
            ) : businesses.map(biz => (
              <tr key={biz.id} className="hover:bg-gray-800/30 transition-colors">
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                      {biz.name.charAt(0)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-white">{biz.name}</p>
                      {biz.gstin && <p className="text-xs font-mono text-gray-500">{biz.gstin}</p>}
                      {biz.city && <p className="text-xs text-gray-600">{biz.city}, {biz.state}</p>}
                    </div>
                  </div>
                </td>
                <td className="px-4 py-4">
                  <p className="text-sm text-gray-300">{biz.owner.name}</p>
                  <p className="text-xs text-gray-500">{biz.owner.email}</p>
                </td>
                <td className="px-4 py-4">
                  <span className={`text-xs px-2 py-0.5 rounded-full border ${planColors[biz.plan?.name || 'Free'] || 'bg-gray-800 text-gray-400 border-gray-700'}`}>
                    {biz.plan?.name || 'Free'}
                  </span>
                </td>
                <td className="px-4 py-4">
