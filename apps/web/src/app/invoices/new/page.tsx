    ...
    <>
      ...
      <div className="p-4 lg:p-6 xl:p-8 w-full max-w-full">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          {/* Sidebar */}
          <div>
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm sticky top-6 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
                <Calculator className="h-4 w-4 text-indigo-500" />
                <h2 className="text-sm font-semibold text-gray-800">Tax Summary</h2>
              </div>
              <div className="p-5 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">Subtotal</span>
                  <span className="text-sm font-semibold text-gray-800">&#8377;{totals.subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">Taxable Amount</span>
                  <span className="text-sm font-semibold text-gray-800">&#8377;{totals.taxableAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                </div>
                  ...
                  {!isInterState ? (
                    <>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm text-gray-700">&#8377;{totals.cgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-500">SGST</span>
                        <span className="text-sm text-gray-700">&#8377;{totals.sgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    </>
                  ) : (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500">IGST</span>
                      <span className="text-sm text-gray-700">&#8377;{totals.igst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Total highlight */}
              <div className="mx-4 mb-4 rounded-2xl bg-gradient-to-br from-indigo-600 to-indigo-700 px-5 py-4 text-white">
                <p className="text-xs font-semibold uppercase tracking-wider text-indigo-200 mb-1">Invoice Total</p>
                <p className="text-3xl font-bold tracking-tight">&#8377;{totals.total.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
              </div>
              ...
              <div className="px-4 pb-5 space-y-2">
                ...
                <button
                  onClick={() => handleSave()}
                  disabled={isLoading || invoiceLimitReached}
                  className="w-full rounded-xl border border-gray-200 bg-white py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-50 transition-all"
                >
                  Save as Draft
                </button>
                <button
                  onClick={() => handleSave('SENT')}
                  disabled={isLoading || invoiceLimitReached}
                  className="w-full rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 py-2.5 text-sm font-semibold text-white hover:from-indigo-700 hover:to-indigo-800 flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-sm"
                >
                  {isLoading ? (
